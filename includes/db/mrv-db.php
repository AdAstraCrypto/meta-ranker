...
//Insert voter data
    ...
    public function wp_insert_rows($row_arrays, $wp_table_name, $update = false, $primary_key = null)
    {
        ...
        $selectQuery = "SELECT * FROM {$wp_table_name} WHERE list_id = %s AND user_wallet = %s";
        $selectResult = $wpdb->get_row($wpdb->prepare($selectQuery, $row_arrays['list_id'], $row_arrays['user_wallet']));
        ...
        if ($selectResult) {
            ...
            $updateQuery .= " WHERE list_id = %s AND user_wallet = %s";
    
            $updateValues[] = $row_arrays['list_id'];
            $updateValues[] = $row_arrays['user_wallet'];
