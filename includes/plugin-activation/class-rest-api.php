...
final class MetaRankerRestApi
{
     ...
     * @return bool
     */
    public static function registerSite($plugin, $email, $wallet)
    {

        $resp = self::request('/v2/auth/register', 'POST', ['email' => $email, 'wallet' => ['address' => $wallet, 'ticker' => "ETH"]], self::getAuthToken($plugin));
        error_log("inside " . __FUNCTION__);error_log(print_r($resp, true));
        if ($resp['status'] === 200) {
            $body = json_decode($resp['body']);
            return $body->status;
        } else {
